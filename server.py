...
class UdpServer:
    ...
    def start(self):
        ...
        self.receiver.start()
        print(f'Сервер запущен на порту: {self.server_port}.\nВремя смещения: {self.time_offset}сек\n')
...
def parse_args(args):
    parser = argparse.ArgumentParser(description="SNTP сервер, позволяющий отвечат на запросы со смещением по времени.")

    parser.add_argument('-d', action='store', dest='time', type=int, default=0,
                        help='Смещение от правильного времени в секундах. '
                             'Может быть положительным/отрицательным')

    parser.add_argument('-p', '--port', action='store', type=int, default=123, help='Порт сервера.')

    args = parser.parse_args(args)

    if args.port < 1 or args.port > 65535:
        print('Введите корректный порт', file=sys.stderr)
        exit(2)
